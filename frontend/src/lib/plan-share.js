...
//
// Two jobs:
//  1. A small, self-contained file a friend can import into THEIR LiftBoi — just the
...
export function buildPlanBundle(S, name) {
  ...
  return { liftboi_plan: PLAN_FMT, exported: todayISO(), name: name || '', week, routines, customEx }
}
...
export function parsePlan(raw) {
  ...
  if (!data || !data.liftboi_plan || !Array.isArray(data.routines)) {
    throw new Error(t('this isn’t an LiftBoi plan file'))
  }
...
<body><div class="doc">
  ...
  ${body}
  <footer>${esc(t('Made with LiftBoi'))} · liftboi.duarte-santos.ch</footer>
</div></body></html>`
}
