...
LiftBoi ships in two flavors from the same codebase:
...
telemetry. State is mirrored from `localStorage` into `liftboi-state.json` in the app's
...
you can instead **connect it to a self-hosted LiftBoi server** — your data then lives there,
...
- **iOS:** a Mac with Xcode 15+ and CocoaPods (`brew install cocoapods`). A free Apple ID
  ...
  for App Store distribution, which LiftBoi doesn't do.
...
LiftBoi's mobile app is not on the Play Store or App Store, and that's a choice: no store
...
- **[liftboi.duarte-santos.ch](https://liftboi.duarte-santos.ch)** — the download page.
- **[GitLab's package registry](https://github.com/5anku/LiftBoi/-/packages)** — every
  build under `liftboi-android/<version>/`, with a `.sha256` beside it. Direct link, no login:
  `https://gitlab.com/api/v4/projects/85678327/packages/generic/liftboi-android/<version>/LiftBoi-<version>.apk`
- **[The GitLab release](https://github.com/5anku/LiftBoi/-/releases)** for that version,
...
keytool -genkeypair -keystore my.keystore -alias liftboi -keyalg RSA -validity 10950

# align + sign (zipalign/apksigner ship with the Android SDK build-tools)
zipalign -f -p 4 app-release-unsigned.apk aligned.apk
apksigner sign --ks my.keystore --ks-key-alias liftboi --out LiftBoi.apk aligned.apk
```
...
- **License:** LiftBoi is AGPL-3.0, which by itself sits badly with app-store terms of
