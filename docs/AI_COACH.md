...
hardware can hold and it appears in the picker on the next refresh; nothing in LiftBoi pins
...
Complete its normal browser sign-in and copy the token it prints. LiftBoi never opens or handles
...
**The instance owner is responsible for their provider's terms.** LiftBoi does not interpret
...
- **Use my self-hosted LiftBoi.** Pair the phone with your instance (the same pairing flow as
