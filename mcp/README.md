...
application (Claude Desktop, Cursor, Cline, Continue, etc.) read your LiftBoi profile —
...
session secrets — it can only read the same `state-<uid>.json` files the LiftBoi api already
writes.
...
LIFTBOI_UID=<your-uid> LIFTBOI_DATA=/path/to/LiftBoi/data node src/index.js
```
...
```jsonc
{
  "mcpServers": {
    "liftboi": {
      "command": "node",
      "args": ["/absolute/path/to/LiftBoi/mcp/src/index.js"],
      "env": {
        "LIFTBOI_DATA": "/absolute/path/to/LiftBoi/data",
        "LIFTBOI_UID": "<your-uid>"   // optional — auto-detected if you have one profile
      }
    }
  }
}
...
Restart the client; you should see the LiftBoi tools appear with "serving profile \<name\>" on
the server's stderr.
...
AGPL-3.0-or-later, same as LiftBoi.
