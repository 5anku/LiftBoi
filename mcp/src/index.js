#!/usr/bin/env node
/* LiftBoi MCP server — stdio transport. The LLM client (Claude Desktop, Cursor, …) spawns
...
// Fail fast on bad config so a misnamed LIFTBOI_DATA doesn't silently answer every call with
...
try {
  init()
  const u = getUser()
  console.error(`[liftboi-mcp] serving profile ${u.name} (${u.id})`)
} catch (e) {
  console.error(`[liftboi-mcp] ${e.message}`)
