...
LiftBoi is a self-hosted app: you run the server, you hold the data. This file says which
...
<https://github.com/5anku/LiftBoi/-/issues/new> and tick **"This issue is confidential"**
...
> `github.com/DuarteSantos8/LiftBoi/security/advisories/new` no longer resolves.
...
- **The published images** `registry.github.com/5anku/LiftBoi/api` and `/web`.
...
Read this before hosting LiftBoi for anyone other than yourself.
...
- **Nothing in `./data` is encrypted.** It holds `db.json` (users, passkey public keys, push
  ...
  `secret` can mint a valid session cookie for any account. **If you host LiftBoi for other
